...
export function NodeCard({ node, isRoot, onEdit, onDelete, onAddChild, onDragStart, className }: NodeCardProps) {
  ...
  if (node.customBackgroundColor) {
    ...
    currentHeaderClasses = cn(headerBaseClasses, 'bg-transparent'); // Make header transparent to show node color
    currentButtonTextClass = "text-[hsl(var(--card-foreground))] dark:text-[hsl(var(--card-foreground))]";

    // Use the HSL variable directly for the description background with opacity
  ...
  return (
    <div
      id={`node-${node.id}`}
      className={cn(currentCardClasses, "node-card-draggable", className)} 
      style={cardStyle}
      draggable
    ...
    >
      ...
      {/* Add a minimum height if there's no description or image, to ensure the card has some body */ }
