...
export function MindmapEditor({ mindmapId }: MindmapEditorProps) {
  ...
  const handleUndo = useCallback(() => {
    ...
    const previousDataFromStack = undoStack[undoStack.length - 2]; // The one before the current state
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      if (document.activeElement && ['INPUT', 'TEXTAREA'].includes(document.activeElement.tagName)) {
        return; // Don't trigger undo/redo if typing in an input field
      }
  ...
  const handleRecenterView = useCallback(() => {
    ...
    if (allNodesArray.length === 0) {
      ...
      const newPanY = (viewportRect.height - (canvasNumericHeight * targetScale) / 2) + (canvasNumericHeight * targetScale / 2) - (100 + 50) * targetScale; // Approx initial Y offset
    ...
    const newScale = Math.max(0.25, newScaleUnclamped);
  ...
  const handlePanMouseDown = useCallback((event: React.MouseEvent<HTMLDivElement>) => {
    ...
    if ((event.target as HTMLElement).closest('.node-card-draggable') || (event.target as HTMLElement).closest('button') || (event.target as HTMLElement).closest('input') || (event.target as HTMLElement).closest('textarea')) {
      return;
    }
    event.preventDefault();
    setIsPanning(true);
  ...
  const handleAddChildNode = useCallback((parentId: string) => {
    ...
    const tempNewNode: NodeData = {
      ...
      customBackgroundColor: parentNode.customBackgroundColor, // Inherit parent's custom color for dialog default
    };
  ...
  const handleDropOnCanvas = useCallback((event: React.DragEvent<HTMLDivElement>) => {
    ...
    try {
      ...
      if (!dragPayload || dragPayload.logicalDragOffsetX === undefined) { console.error("No valid drag data found"); return; }
  ...
  const svgKey = allNodes.map(n => `${n.id}-${n.x}-${n.y}-${n.parentId}-${(n.childIds || []).join(',')}-${scale}-${pan.x}-${pan.y}-${n.customBackgroundColor}`).join('|');
      ...
      <div className="flex flex-col h-full flex-grow w-full">
        ...
        <div className="flex-grow flex items-center justify-center p-0 bg-muted/20 overflow-hidden">
          <div
            ...
            style={{ width: `1200px`, height: `800px`, overflow: 'hidden' }}
            onMouseDown={handlePanMouseDown}
            onTouchStart={handleTouchStart}
            onTouchMove={handleTouchMove}
            onTouchEnd={handleTouchEnd}
            onDragOver={handleDragOverCanvas} // For dropping nodes from outside (if ever needed)
            onDrop={handleDropOnCanvas} // Handles dropping nodes within
          >
            <div
              ref={canvasContentRef}
              className="bg-card border-2 border-dashed border-sky-300" // Logical canvas has the border and bg now
              style={{
                width: CANVAS_CONTENT_WIDTH_STR, height: CANVAS_CONTENT_HEIGHT_STR,
            ...
            >
              ...
              >
                {allNodes.map(node => {
                  ...
                  return (
                    <path key={`${parentNode.id}-${node.id}`} d={pathData} stroke={strokeColor} strokeWidth={2 / scale} fill="none" />
                  );
                })}
              </svg>
              {allNodes.map((node) => (
                <NodeCard
                  ...
                  onDragStart={handleNodeDragStart} className="node-card-draggable"
                />
              ))}
              {allNodes.length === 0 && (
